...
class Constants:
    ...
    def get_override_log_path(main_folder: str):
        return os.path.join(main_folder, Constants.OVERRIDE_LOG_FILE_NAME)

    # modified
    @staticmethod
    def get_exclude_from_tagging_file_path(main_folder: str):
        return os.path.join(main_folder, Constants.EXCLUDE_FROM_TAGGING_FILE_NAME)
    ...
    def wrap_class(cls):
        for (attr_name, attr_value) in cls.__dict__.items():
            if (
                ...
                or (not callable(attr_value) and isinstance(attr_value, staticmethod))
            ):
    ...
    def wrap_func(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                LoggerHelper.write_error(
                    f"An Unhandled exception that started in the line {code_line} was occurred,\n"
                    f"Exception is:\n"
                    f"{trace}\n\n",
                    code_line,
                )
...
class OverrideTagsTemplatesCreator:
    def __init__(self, tags_dict: dict, terraform_version: str):
        ...
        self._terraform_syntax = TerraformSyntaxVersion.get_terraform_syntax(
            terraform_version
        )
        ...
        if not self.torque_tags_dict:
            LoggerHelper.write_error(
                f"Didn't get tags dict, exiting the tagging process"
            )
            return
    ...
    def get_single_group_string(self, single_str: str):
        # In version 0.11.x and below using terraform syntax like "map", "merge", "concat", and more
        # must be wrapped in "${}"
        single_group_match = RegexHelper.get_single_group_match_from_regex_result(
            text=single_str, pattern=RegexHelper.SINGLE_VAR_PATTERN
        )
        ...
        if (
            single_group_match is None
            and self._terraform_syntax == TerraformSyntaxVersion.zero_twelve_and_above
        ):
            return single_str
    ...
    def _init_torque_tags_flat_map(self):
        ...
        for key, value in self.torque_tags_dict.items():
            torque_tags_to_list.append(
                f'"{key}"{self._map_key_value_separator}"{value}"'
            )
    ...
    def _init_torque_autoScaling_tags_flat_maps_list(self):
        ...
        for key, value in self.torque_tags_dict.items():
            map_value = (
                '"key"{0}"{1}","value"{0}"{2}","propagate_at_launch"{0}"true"'.format(
                    self._map_key_value_separator, key, value
                )
            )
        ...
        self.torque_autoScaling_tags_flat_maps_list = self._get_list_tags_template(
            list_of_maps_value
        )

    def _get_basic_override_tags_template(
        self, resource_type: str, resource_name: str, tags: str
    ) -> str:
        tags_label = (
            Constants.LABELS
...
\n""".format(
    ...
    def get_merge_tags_template(
        self, resource_type: str, resource_name: str, client_str_tags: str
    ) -> str:
        ...
        return self._get_basic_override_tags_template(
            resource_type=resource_type,
            resource_name=resource_name,
            tags=self._get_terraform_syntax_string_template(merge_str),
        )

    def get_concat_tags_template(
        self, resource_type: str, resource_name: str, client_str_tags: str
    ) -> str:
    ...
    def get_torque_tags_with_client_dict_tags_template(
        self, resource_type: str, resource_name: str, client_dict_tags: dict
    ) -> str:
    ...
    def get_torque_tags_with_autoscaling_client_dict_tags_template(
        self, resource_type: str, resource_name: str, client_list_dict_tags: list
    ) -> str:
        ...
        for tag in list_of_tags_dict:
            all_tags_in_str_list.append(
                f"{{key = \"{tag['key']}\", value = \"{tag['value']}\","
...
class Hcl2Parser:
    ...
    def get_tf_file_as_dict(tf_file_path: str) -> dict:
        try:
            with (open(tf_file_path)) as client_tf_file:
    ...
    def get_terraform_resource_safely(full_resource_object: dict) -> TerraformResource:
        ...
        if (
            full_resource_object[resource_type] is None
            or not full_resource_object[resource_type].keys()
        ):
            return None
        ...
        if tags:
            ...
            if type(tags) is str:
                tags = tags.replace("'", '"').replace(
                    ",,", ","
                )  # due to bug in hcl2 library

        return TerraformResource(
            resource_type=resource_type, resource_name=resource_name, tags=tags
        )
    ...
    def get_tf_file_resources(tf_file_path: str) -> list[TerraformResource]:
        ...
        for resource_object in tf_as_dict.get("resource", []):
            tf_file_resources.append(
                Hcl2Parser.get_terraform_resource_safely(resource_object)
            )
...
class RegexHelper:
    ...
    UNSUPPORTED_TAGS_OR_LABELS_PATTERN_1 = r"^\bError: Unsupported argument\b[\n]*.*\bresource\b[ ]*\"(.*?)\".*[\n]*.*[\n]*.*\"(?:\btags\b|\blabels\b)\"[ ]*\bis not expected here\b"
    ...
    UNSUPPORTED_TAGS_OR_LABELS_PATTERN_2 = (
        r"^\bError\b[ ,:]*(.*?)\..*\binvalid or unknown\b.*\btags\b"
    )
    ...
    @staticmethod
    def get_all_group_match_from_regex_result(
        ...
        for pattern in patterns:
            matches = re.findall(
                pattern, text, re.M
            )  # re.M = multi line matches (i.e include \n in the search)
...
class ResourcesTagger:
    def __init__(
        self, tf_file_info: FileInfo, tags_creator: OverrideTagsTemplatesCreator
    ):
    ...
    def _get_override_file_path(self):
        ...
        override_file_path = os.path.join(
            self.tf_file_info.file_dir, f"{file_without_ext}_override.tf"
        )
        return override_file_path

    def _get_torque_tags_with_client_string_tags(
        self, terraform_resource: TerraformResource
    ) -> str:
        ...
        # MERGED_TAGS_PATTERN
        parsed_client_tags = RegexHelper.get_single_group_match_from_regex_result(
            text=client_tags, pattern=RegexHelper.MERGED_TAGS_PATTERN
        )
        if (
            not parsed_client_tags
        ):  # if not find concat pattern then try to fall back to single var pattern
    ...
    def _get_torque_tags_with_autoscaling_client_string_tags(
        self, terraform_resource: TerraformResource
    ) -> str:
        client_tags = terraform_resource.tags.replace("\n", "")
        ...
        parsed_client_tags = RegexHelper.get_single_group_match_from_regex_result(
            text=client_tags, pattern=RegexHelper.CONCAT_LIST_TAGS_PATTERN
        )
        if (
            not parsed_client_tags
        ):  # if not find concat pattern then try to fall back to single var pattern
    ...
    def _get_torque_tags_with_client_dict_tags(
        self, terraform_resource: TerraformResource
    ) -> str:
    ...
    def _get_torque_tags_with_autoscaling_client_dict_tags(
        self, terraform_resource: TerraformResource
    ) -> str:
    ...
    def _add_tags_to_override_file(
        self, override_file_stream, terraform_resource: TerraformResource
    ):
            ...
            if terraform_resource.resource_type == "aws_autoscaling_group":
                ...
                if not terraform_resource.tags or type(terraform_resource.tags) is list:
                    override_file_stream.write(
                        self._get_torque_tags_with_autoscaling_client_dict_tags(
                            terraform_resource
                        )
                    )
                # i.e tags is most likely = "${concat(<List_of_maps1>,<List_of_maps2>,....)}" or "${<List_of_maps>}"
                    ...
                    override_file_stream.write(
                        self._get_torque_tags_with_autoscaling_client_string_tags(
                            terraform_resource
                        )
                    )
            else:
                ...
                if not terraform_resource.tags or type(terraform_resource.tags) is dict:
                    override_file_stream.write(
                        self._get_torque_tags_with_client_dict_tags(terraform_resource)
                    )
                # i.e tags is most likely = "${merge(<map1>,<map2>,....)}" or "${<map>}"
                elif type(terraform_resource.tags) is str:
                    override_file_stream.write(
                        self._get_torque_tags_with_client_string_tags(
                            terraform_resource
                        )
                    )

    def create_override_file(self, untaggable_resources_types: list[str] = []):
        tf_file_resources = Hcl2Parser.get_tf_file_resources(
            self.tf_file_info.file_path
        )
        if not tf_file_resources or len(tf_file_resources) == 0:
            LoggerHelper.write_info(
                f"No need to create override file to {self.tf_file_info.file_name}"
        ...
        LoggerHelper.write_info(
            f"Override file was created to {self.tf_file_info.file_name}"
...
def _get_untaggable_resources_types_from_plan_output(text: str) -> list[str]:
    untaggable_resources = RegexHelper.get_all_group_match_from_regex_result(
        text=text,
        patterns=[
            RegexHelper.UNSUPPORTED_TAGS_OR_LABELS_PATTERN_1,
...
# modified
def start_tagging_terraform_resources(
    main_dir_path: str,
    logger,
    tags_dict: dict,
    ...
    exclude_from_tagging_file_path = Constants.get_exclude_from_tagging_file_path(
        main_dir_path
    )
    ...
    LoggerHelper.write_info(
        f"Trying to preform terraform init & plan in the directory '{tfs_folder_path}'"
        " in order to check for any validation errors in tf files"
    )
    stdout, stderr, return_code = _perform_terraform_init_plan(
        tfs_folder_path, inputs_dict
    )  # modified
    if return_code != 0 or stderr:
        LoggerHelper.write_error(
            "Exit before the override procedure began because the init/plan failed."
    ...
    all_tf_files_without_overrides = [
        file for file in all_tf_files if not file.file_name.endswith("_override.tf")
    ]
    ...
    if os.path.exists(exclude_from_tagging_file_path):
        ...
        LoggerHelper.write_info(
            f"User decided to exclude the following resources types from tagging: {untaggable_resources_types}"
        )
    ...
    LoggerHelper.write_info(
        f"Try to create override files to those tf's': {all_tf_files_without_overrides}\n"
    )
    ...
    if stderr:
        LoggerHelper.write_warning(
            "Errors founds in plan output. The errors are:\n\n" f"{stderr}\n"
        )
        LoggerHelper.write_info("Trying to search for any untaggable resources")
        untaggable_resources_types_from_plan = (
            _get_untaggable_resources_types_from_plan_output(text=stderr)
        )
        ...
        if untaggable_resources_types_from_plan:
            LoggerHelper.write_warning(
                f"The following resources (in the tf files) are untaggable:"
            ...
            LoggerHelper.write_info(
                f"Trying to preform one final terraform init & plan in the directory '{tfs_folder_path}'"
            ...
            if return_code != 0 or stderr:
                LoggerHelper.write_error(
                    "Errors were found in the last validation check:"
                    f" (Return_code is {return_code})"
                    f"\n\nErrors are:\n{stderr}\n"
                )
                LoggerHelper.write_error(
                    "Tagging terraform resources operation has FAILED !!!!!"
                )
                exit(1)

        else:
            LoggerHelper.write_warning(
                "No untaggable resources were found, but errors in plan file do exists"
            )
            LoggerHelper.write_error(
                "Tagging terraform resources operation has FAILED !!!!!"
            )
    ...
    LoggerHelper.write_info(
        "Successfully finish creating override files to tf files\n\n\n\n"
    )
