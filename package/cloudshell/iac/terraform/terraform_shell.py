...
class TerraformShell:
    ...
    def __init__(
        self,
        driver_context: ResourceCommandContext,
        logger: logging.Logger = None,
        config: TerraformShellConfig = None,
    ):
    ...
    def execute_terraform(self):
        ...
        with nullcontext(self._logger) if self._logger else LoggingSessionContext(
            self._context
        ) as logger:
            shell_helper = ObjectFactory.create_shell_helper(
                self._tf_service, self._context, self._config, logger
            )
            sandbox_data_handler = SandboxDataHandler(shell_helper)
            tf_working_dir = LocalDir.prepare_tf_working_dir(
                logger, sandbox_data_handler, shell_helper
            )
    ...
    def _execute_procedure(
        ...
        try:
            tf_proc_executer = ObjectFactory.create_tf_proc_executer(
                self._config, sandbox_data_handler, shell_helper, tf_working_dir
            )
    ...
    def destroy_terraform(self):
        ...
        with nullcontext(self._logger) if self._logger else LoggingSessionContext(
            ...
            sandbox_data_handler = SandboxDataHandler(shell_helper)
            self._validate_remote_backend_or_existing_working_dir(
                sandbox_data_handler, shell_helper
            )
    ...
    def _destroy_procedure(
        ...
        if not tf_working_dir:
            self._handle_error_output(
                shell_helper, "Destroy failed due to missing local directory"
            )
        ...
        try:
            ...
            if tf_proc_executer.can_destroy_run():
                ...
                self._handle_error_output(
                    shell_helper, "Destroy blocked because APPLY was not yet executed"
                )
        finally:
    ...
    def _validate_remote_backend_or_existing_working_dir(
        self, sandbox_data_handler, shell_helper
    ):
        if not shell_helper.attr_handler.get_attribute(
            ATTRIBUTE_NAMES.REMOTE_STATE_PROVIDER
        ) and not LocalDir.does_working_dir_exists(
    ...
    def _using_remote_state(shell_helper) -> bool:
        return bool(
            shell_helper.attr_handler.get_attribute(
                ATTRIBUTE_NAMES.REMOTE_STATE_PROVIDER
            )
        )
