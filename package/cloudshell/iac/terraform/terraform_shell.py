...
from cloudshell.iac.terraform.constants import DESTROY_STATUS, DESTROY_PASSED, \
    ATTRIBUTE_NAMES, OPA_VALIDATION_COMMAND
...
class TerraformShell:
    ...
    def execute_terraform(self):
        ...
        with nullcontext(self._logger) \
                if self._logger else LoggingSessionContext(self._context) as logger:
        ...
        try:
            ...
            if tf_proc_executer.can_execute_run():
                ...
                self._handle_error_output(shell_helper,
                                          "This Terraform Module has been successfully "
                                          "deployed but "
                                          "destroy failed. Please destroy successfully "
                                          "before running "
                                          "execute again."
                                          )
        finally:
    ...
    def _destroy_procedure(self,
        ...
        try:
            ...
            if self._using_remote_state(shell_helper) \
                    or self._destroy_passed(sandbox_data_handler):
                LocalDir.delete_local_temp_dir(sandbox_data_handler, tf_working_dir)

    def _validate_remote_backend_or_existing_working_dir(self,
                                                         sandbox_data_handler,
                                                         shell_helper
                                                         ):
