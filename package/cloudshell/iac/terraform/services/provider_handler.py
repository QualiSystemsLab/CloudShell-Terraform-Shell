...
class ProviderHandler:
    ...
    def initialize_provider(self, shell_helper: ShellHelperObject):
        clp_resource_name = shell_helper.attr_handler.get_attribute(
            ATTRIBUTE_NAMES.CLOUD_PROVIDER
        )
        ...
        try:
            ...
            shell_helper.logger.error(
                f"Error Setting environment variables -> {str(e)}"
            )
            raise
    ...
    def _set_cloud_env_vars(
        ...
        if clp_res_model in [AWS1G_MODEL, AWS2G_MODEL]:
            clp_handler = AWSCloudProviderEnvVarHandler(
                clp_res_model, clp_resource_attributes, shell_helper
            )
