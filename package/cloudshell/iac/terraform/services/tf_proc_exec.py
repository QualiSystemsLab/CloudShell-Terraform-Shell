...
class TfProcExec:
    def __init__(
        ...
        sb_data_handler: SandboxDataHandler,
        ...
        self._exec_output_log = _create_logger(
            log_group=shell_helper.sandbox_id,
            log_category="QS",
            log_file_prefix=f"TF_EXEC_LOG_{dt}",
        )
    ...
    def tag_terraform(self) -> None:
        try:
            ...
            apply = self._shell_helper.attr_handler.get_attribute(
                ATTRIBUTE_NAMES.APPLY_TAGS
            )
            if apply and not strtobool(apply):
                self._shell_helper.logger.info(
                    "Skipping Adding Tags to Terraform Resources"
                )
                self._shell_helper.sandbox_messages.write_message(
            ...
            check_tag_input = self._shell_helper.attr_handler.get_attribute(
                ATTRIBUTE_NAMES.CT_INPUTS
            )
            if check_tag_input:
                custom_tags_inputs = (
                    self._input_output_service.get_tags_from_custom_tags_attribute()
                )
            else:
                custom_tags_inputs = {}
            ...
            if len(tags_dict) > 50:
                raise ValueError(
                    "AWS and Azure have a limit of 50 tags per resource, you have "
                    + str(len(tags_dict))
                )
            ...
            terraform_version = self._shell_helper.attr_handler.get_attribute(
                ATTRIBUTE_NAMES.TERRAFORM_VERSION
            )
            ...
            self._shell_helper.sandbox_messages.write_error_message(
    ...
    def save_terraform_outputs(self):
        try:
            ...
            cmd = ["output", "-json"]
            tf_exec_output = self._run_tf_proc_with_command(
                cmd, OUTPUT, write_to_log=False
            )
    ...
    def _run_tf_proc_with_command(
        self, cmd: list, command: str, write_to_log: bool = True
    ) -> str:
        ...
        try:
            ...
            self._shell_helper.logger.error(
                f"Error Running Terraform {command} {clean_output}"
            )
            raise TerraformExecutionError(
    ...
    def _set_service_status(self, status: str, description: str):
        self._shell_helper.live_status_updater.set_service_live_status(
            self._shell_helper.tf_service.name, status, description
        )
