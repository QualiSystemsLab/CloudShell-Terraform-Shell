...
from cloudshell.iac.terraform.constants import (
    OS_TYPES,
    TERRAFORM_LATEST_URL,
    TERRAFORM_URL,
)


class TfExecDownloader:
    ...
    def download_terraform_executable(tf_workingdir: str, version="latest"):
        ...
        valid_version_regex = re.compile(
            r"^([0-9]{1})\.([1-9]{0,1}[0-9]{1})\.([1-9]{0,1}[0-9]{1})$"
        )
        ...
        if not os.path.exists(tf_workingdir):
            raise ValueError(
                f"Target path: {tf_workingdir} does not exist. Cannot be sym link."
            )
        if valid_version_regex.match(version) is None:
            raise ValueError(
                f"Version {version} is not a valid format. examples 1.0.0, 0.15.2, 0.12.15"
            )
        if sys.platform not in OS_TYPES:
            raise ValueError(
                "Could not find OS type. Must be 64 bit and Windows, Ubuntu, or CentOS/Redhat."
            )
