...
class Downloader:
    ...
    def download_terraform_module(self) -> str:
        url = self._shell_helper.attr_handler.get_attribute(
            ATTRIBUTE_NAMES.GIT_TERRAFORM_MODULE_URL
        )
        if not url:
            raise ValueError(
                f"Must populate attribute '{ATTRIBUTE_NAMES.GIT_TERRAFORM_MODULE_URL}'"
            )

        token_enc = self._shell_helper.attr_handler.get_attribute(
            ATTRIBUTE_NAMES.GIT_TOKEN
        )
        ...
        provider = self._shell_helper.attr_handler.get_attribute(
            ATTRIBUTE_NAMES.GIT_PROVIDER
        )
        downloader = self._downloader_factory(
            provider, logger=self._shell_helper.logger
        )

        # download repo and return working dir
        self._shell_helper.sandbox_messages.write_message(
            "downloading Terraform module from repository..."
        )
    ...
    def download_terraform_executable(self, tf_workingdir: str) -> None:
        try:
            ...
            TfExecDownloader.download_terraform_executable(
                tf_workingdir,
                self._shell_helper.attr_handler.get_attribute(
