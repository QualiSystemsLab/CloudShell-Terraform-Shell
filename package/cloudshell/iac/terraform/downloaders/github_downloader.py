...
class GitHubScriptDownloader(GitScriptDownloaderBase):
            ...
            if tf_response.status_code == 200:
                ...
                if repo_response.status_code == 200:
                    working_dir = self._prepare_working_dir(
                        path_in_repo, repo_response, tf_response_json, url_data
                    )
                    return working_dir
                else:
                    raise Exception(
                        f"Error Downloading/Extracting: Download code for repo={repo_response.status_code}"
                    )
            else:
                raise Exception(
                    f"Error Downloading/Extracting - Download code for module url (Check Token and URL)"
                    f"{tf_response.status_code}"
                )
        except Exception as e:
            self.logger.error(
                f"There was an error downloading and extracting the repo. {str(e)}"
            )
            raise

    def _prepare_working_dir(
        self, path_in_repo, repo_response, tf_response_json, url_data
    ):
        ...
        os.rename(
            os.path.join(repo_temp_dir, commit_folder_in_zip),
            os.path.join(repo_temp_dir, "REPO"),
        )
    ...
    def _raise_url_syntax_error(self) -> None:
        raise ValueError(
            "Provided GitHub URL is not in the correct format. "
    ...
    def _extract_data_from_url(self, url: str, branch_attr: str = ""):
        ...
        if matching:
            ...
            return GitHubFileData(
                account_id, repo_id, branch_id, path, api_zip_dl_url, api_tf_dl_url
            )
        else:
            self._raise_url_syntax_error()
