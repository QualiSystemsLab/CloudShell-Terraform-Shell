...
from tests.constants import (
    GITHUB_TF_PUBLIC_HELLO_URL_FILE,
...
class TestTerraformDownloader(TestCase):
    def setUp(self) -> None:
        ...
        service_resource = GenericTerraformService.create_from_context(
            self.integration_data.context
        )
    ...
    def _test_download_terraform_module(self, url: str, branch: str):
        ...
        self._driver_helper.tf_service.attributes[f"{SHELL_NAME}.Branch"] = branch
    ...
    def test_public_and_private_hello_dl(self):
        ...
        self._test_download_terraform_module(
    ...
    def test_download_terraform_executable(self):
        ...
        self.assertTrue(
            os.path.exists(os.path.join(tf_workingdir, TERRAFORM_EXEC_FILE))
        )
        self.assertTrue(
            os.access(os.path.join(tf_workingdir, TERRAFORM_EXEC_FILE), os.X_OK)
        )
