...
class TestRealTerraformExecuteDestroy(TestCase):
    def setUp(self) -> None:
        ...
        self.integration_data1 = IntegrationData(
            os.environ.get("SB_SERVICE_ALIAS1"), is_api_real=True
        )
        self.integration_data2 = IntegrationData(
            os.environ.get("SB_SERVICE_ALIAS2"), is_api_real=True
        )
    ...
    def test_execute_dual_mssql(self):
        ...
        try:
            self.run_execute(
                pre_exec_function=self.pre_exec_azure_mssql,
                integration_data=self.integration_data1,
            )
        ...
        try:
            self.run_execute(
                pre_exec_function=self.pre_exec_azure_mssql,
                integration_data=self.integration_data2,
            )
        except Exception as e:
            pass
        self.run_destroy(
    ...
    def _set_attribute_on_service(
        self, attr_name: str, attr_value: str, integration_data: IntegrationData
    ):
