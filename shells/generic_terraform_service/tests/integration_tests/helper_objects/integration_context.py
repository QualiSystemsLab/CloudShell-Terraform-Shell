...
class IntegrationData:
    def __init__(
        self, service_name: str, is_api_real: bool = True, mock_api: Mock = None
    ):
    ...
    def _set_context(self, is_api_real: bool):
        ...
        self.context.connectivity.admin_auth_token = (
            self.api.authentication.xmlrpc_token
        )
        ...
        if is_api_real:
            ...
            self.context.resource.attributes = (
                ServiceAttributesFactory.create_empty_attributes()
            )
    ...
    def set_context_resource_attributes_from_cs(
        self, the_only_attribute_to_update: str = ""
    ):
        services = self.api.GetReservationDetails(
            self._env_vars.cs_res_id, disableCache=True
        ).ReservationDescription.Services
        for service in services:
            if service.Alias == self._env_vars.sb_service_alias:
                for attribute in service.Attributes:
                    if (
                        the_only_attribute_to_update
                        and attribute.Name == the_only_attribute_to_update
                    ):
                        self.context.resource.attributes[
                            attribute.Name
                        ] = attribute.Value
    ...
    def create_tf_shell(self):
        self._config = TerraformShellConfig(
