...
from tests.integration_tests.helper_services.service_attributes_factory import (
    ServiceAttributesFactory,
)
...
class TestMockTerraformExecuteDestroy(TestCase):
    ...
    def setUp(self, patched_api) -> None:
        ...
        self.mock_api.GetReservationDetails.return_value.ReservationDescription.Services = [
            self._service1,
            self._service2,
        ]
    ...
    def _prepare_integration_data(self):
        self.integration_data1 = IntegrationData(
            self._service1.Alias, False, self.mock_api
        )
        self.integration_data2 = IntegrationData(
            self._service2.Alias, False, self.mock_api
        )
    ...
    def _prepare_mock_api(self):
        ...
        self.mock_api.GetResourceDetails.return_value.ResourceFamilyName = (
            "Cloud Provider"
        )
        self.mock_api.GetResourceDetails.return_value.ResourceModelName = (
            "Microsoft Azure"
        )
        self.mock_api.GetResourceDetails.return_value.ResourceAttributes = [
            ...
            NameValuePair(
                Name="Azure Tenant ID", Value=os.environ.get("AZURE_TENANT_ID")
            ),
            NameValuePair(
                Name="Azure Application ID",
                Value=os.environ.get("AZURE_APPLICATION_ID"),
            ),
            NameValuePair(
                Name="Azure Application Key",
                Value=os.environ.get("AZURE_APPLICATION_KEY_DEC"),
            ),
        ]
    ...
    def run_execute(
        self, pre_exec_function: Callable, integration_data: IntegrationData
    ):
        self.pre_exec_prep(pre_exec_function, integration_data)
        integration_data.tf_shell.execute_terraform()

    def run_destroy(
        self, pre_destroy_function: Callable, integration_data: IntegrationData
    ):
    ...
    @patch("cloudshell.iac.terraform.services.object_factory.CloudShellSessionContext")
    def test_execute_and_destroy_azure_vault(
        self, patch_api, patched_sbdata_handler, can_destroy_run
    ):
    ...
    def pre_destroy_prep(
        self, pre_destroy_function: Callable, integration_data: IntegrationData
    ):
        pre_destroy_function(integration_data)

    def clear_sb_data(self):
        self.integration_data1.api.ClearSandboxData(
    ...
    def pre_exec_azure_mssql(self, integration_data: IntegrationData):
        ...
        self._set_attribute_on_mock_service(
            f"{SHELL_NAME}.UUID", os.environ.get(""), integration_data
        )
        self._prepare_service1_before_execute(integration_data)

    def pre_exec_azure_vault_with_remote_access_key_based(
    ...
    def pre_exec_azure_vault_with_remote_cloud_cred_based(
        self, integration_data: IntegrationData
    ):
        self.pre_exec_azure_vault(integration_data)
        self._set_attribute_on_mock_service(
    ...
    def pre_exec_azure_vault_with_remote_invalid_nonexistent(
        self, integration_data: IntegrationData
    ):
        self.pre_exec_azure_vault(integration_data)
        self._set_attribute_on_mock_service(
    ...
    def pre_exec_azure_vault_with_remote_invalid_wrong(
        self, integration_data: IntegrationData
    ):
        self.pre_exec_azure_vault(integration_data)
        self._set_attribute_on_mock_service(
    ...
    def pre_exec_azure_vault_without_remote(self, integration_data: IntegrationData):
        self.pre_exec_azure_vault(integration_data)
        self._set_attribute_on_mock_service(
            f"{SHELL_NAME}.Remote State Provider", os.environ.get(""), integration_data
        )
    ...
    def pre_destroy(self, integration_data: IntegrationData):
        ...
        for attribute in integration_data.context.resource.attributes:
            if attribute.Name == f"{SHELL_NAME}.UUID":
                attribute.Value = integration_data.tf_shell._tf_service.attributes[
                    f"{SHELL_NAME}.UUID"
                ]
    ...
    @staticmethod
    def _set_attribute_on_mock_service(
        attr_name: str, attr_value: str, integration_data: IntegrationData
    ):
        for attribute in integration_data.context.resource.attributes:
