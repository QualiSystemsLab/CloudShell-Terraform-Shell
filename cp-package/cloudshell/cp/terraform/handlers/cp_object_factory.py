...
class CPObjectFactory:
    @staticmethod
    def create_tf_proc_executer(config: TerraformResourceConfig,
                                sandbox_data_handler: SandboxDataHandler,
                                shell_helper: ShellHelperObject,
                                tf_working_dir: str) -> TfProcExec:
        backend_handler = BackendHandler(shell_helper, tf_working_dir,
                                         sandbox_data_handler.get_tf_uuid())
        input_output_service = InputOutputService(shell_helper, config.inputs_map,
                                                  config.outputs_map)
        tf_proc_executer = TfProcExec(shell_helper, sandbox_data_handler,
                                      backend_handler, input_output_service)
        return tf_proc_executer

    @staticmethod
    def create_shell_helper(resource_config: TerraformResourceConfig,
                            sandbox_id: str,
                            tag_manager: TagsManager,
                            config: TerraformShellConfig,
                            logger: logging.Logger) -> CPShellHelperObject:
        api = resource_config.api
        sandbox_message_service = SandboxMessagesService(api, sandbox_id,
                                                         resource_config.name,
                                                         config.write_sandbox_messages)
        live_status_updater = LiveStatusUpdater(api, sandbox_id,
                                                config.update_live_status)
